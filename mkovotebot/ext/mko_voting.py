...
def get_played_hours(discord_id, in_seconds=False):
    ...
    seconds = db.requestdb(
        f"SELECT (CASE WHEN SUM(played) IS NULL THEN 0 ELSE SUM(played) END) FROM stats_month "
        f'WHERE uuid = "{uuid}" AND date >= CURRENT_DATE - INTERVAL 7 DAY'
    )[0][0]
    if seconds is None:
        return None
    if in_seconds:
...
def get_votes(discord_id, return_users=False):
    res = db.select(
        columns="COUNT(*) as votes.txt",
        where=f"voted_user = {discord_id}",
...
async def logger(ctx=None, type=None, player1=None, player2=None):
    if type == "voted":
        embed = discord.Embed(
            title=texts["voted_title"],
            description=texts["voted_desk"].format(
                player1=player1.mention, player2=player2.mention
            ),
        ...
        embed = discord.Embed(
            title=texts["fvoted_title"],
            description=texts["fvoted_desk"].format(
                player1=player1.mention, player2=player2.mention
            ),
        ...
        embed = discord.Embed(
            title=texts["added_pmember_title"],
        ...
        embed = discord.Embed(
            title=texts["removed_pmember_title"],
        ...
        embed = discord.Embed(
            title=texts["rejected_vote_title"],
            description=texts["rejected_vote_desk"].format(
...
def top():
    voted_users = db.select(
        columns="voted_user, COUNT(*) as votes.txt",
        args="GROUP BY voted_user",
        return_matrix=True,
    )
...
async def move_top(top_to_move, right: bool):
    ...
    if len(top_list) == 0:
        ...
        (top_list_num * vt_len) > len(top_list) > ((top_list_num - 1) * vt_len)
    ):  # сука что это блять значит
    ...
    for user in users:
        ...
        if member:
            ...
            if hours is not None and not hours >= config["hours_to_vote"]:
                ...
                user_to_update = int(
                    db.select(columns="discord_id", where=f"discord_id = {member.id}")[
                        0
                    ]
                )
                await update_member(
                    Pguild.get_member(user_to_update)
                )  # Можно убрать чтобы не перегружать бд, но мне похуй
...
@slash.slash(
    name="vote",
    description="Проголосовать за игрока",
    options=[
        {
            "name": "player",
            "description": "Игрок",
...
async def vote(ctx, player):
    if not channel_check(ctx):
        return "Pepega"
    if (
        config["private_voting"]
        and not fvote_role in ctx.author.roles
        and not ctx.author.id in config["admins"]
    ):
        return False
    ...
    vote = db.select(
        columns="voted_user, last_vote_timestamp", where=f"discord_id = {ctx.author.id}"
    )
    if vote is not None:
        ...
        if int(vote[0]) == player.id:
            ...
            return False
        db.update(
            data=f"voted_user = {player.id}, last_vote_timestamp={int(time.time())}",
...
async def activity(ctx, *, text):
    if ctx.author.id in config["admins"]:
        await bot.change_presence(
            status=discord.Status.do_not_disturb, activity=discord.Game(str(text))
        )
...
async def vote_error(ctx, error, player=None):
    ...
    if isinstance(error, commands.errors.MissingRole):
        ...
        embed = discord.Embed(
            title=errors["err_title"],
            description=errors["Vote TooFewHours"].format(
                hours=config["hours_to_vote"]
            ),
            colour=errors["err_colour"],
        )
...
async def unvote(ctx: SlashContext, player=None):
    ...
    if (
        config["private_voting"]
        and fvote_role not in ctx.author.roles
        and ctx.author.id not in config["admins"]
    ):
        return False
    if not player:
        player = ctx.guild.get_member(ctx.author.id)
    if (
        ctx.author.guild.get_role(config["fvote_role"]) in ctx.author.roles
    ...
    vote = db.select(
        columns="voted_user, last_vote_timestamp", where=f"discord_id = {member.id}"
    )
...
@slash.slash(
    name="fvote",
    description="[ADMIN] Отдать голос игрока за другого игрока",
    options=[
        {
            "name": "player1",
...
async def fvote(ctx, player1, player2):
    ...
    for player in [player1, player2]:
        if player_role not in player.roles:
            embed = discord.Embed(
                title=errors["err_title"],
                description=errors["Fvote PlayerMissingRole"].format(
    ...
    if hours is not None and hours < config["hours_to_vote"]:
        embed = discord.Embed(
            title=errors["err_title"],
            description=errors["Fvote TooFewHours"].format(
                player1=player1.mention, hours=config["hours_to_vote"]
            ),
            colour=errors["err_colour"],
        )
        await ctx.send(f"{ctx.author.mention}", embed=embed)
    ...
    db_vote = db.select(
        columns="voted_user, last_vote_timestamp", where=f"discord_id = {player1.id}"
    )
    if db_vote is not None:
        if int(db_vote[0]) == player2.id:
            embed = discord.Embed(
                title=errors["err_title"],
                description=errors["Fvote AlreadyVoted"].format(
                    player1=player1.mention, player2=player2.mention
                ),
                colour=errors["err_colour"],
            )
            ...
            return False
        db.update(
            data=f"voted_user = {player2.id}, last_vote_timestamp={int(time.time())}",
...
@slash.slash(
    name="vote-info",
    description="Узнать информацию о голосе и о проголосовавших за игрока",
    options=[
        {
            "name": "player",
            "description": "Игрок",
...
async def vote_info(ctx, player=None):
    ...
    if len(voted) > 0:
        ...
        for user in voted:
            users += " " + "<@" + user + ">"
        embed.add_field(
            name=f"За {player.display_name} проголосовало: **{len(voted)}**",
            value=users,
        )
...
@slash.slash(
    name="vote-rcd",
    description="[ADMIN] Сбросить кулдаун у игрока",
    options=[
        {
            "name": "player",
            "description": "Игрок",
...
@slash.slash(
    name="vote-top",
    description="Вывести топ игроков по количеству голосов",
    options=[],
    guild_ids=[config["guild_id"]],
)
...
async def on_reaction_add(reaction: discord.Reaction, user: discord.User) -> None:
    ...
    if (
        emoji == config["reaction_previous"] or emoji == config["reaction_next"]
    ) and not user.bot:
...
async def on_ready():
    await bot.change_presence(
        status=discord.Status.do_not_disturb, activity=discord.Game(config["activity"])
    )
